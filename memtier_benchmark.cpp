...
static void config_print(FILE *file, struct benchmark_config *cfg)
{
    ...
    fprintf(file,
            ...
            cfg->key_stddev, cfg->key_median, cfg->reconnect_interval, cfg->multi_key_get,
...
static int config_parse_args(int argc, char *argv[], struct benchmark_config *cfg)
{
    ...
    static struct option long_options[] = {{"server", 1, 0, 's'},
                                           {"host", 1, 0, 'h'},
                                           {"port", 1, 0, 'p'},
                                           {"unix-socket", 1, 0, 'S'},
                                           {"ipv4", 0, 0, '4'},
...
run_stats run_benchmark(int run_id, benchmark_config *cfg, object_generator *obj_gen)
{
    ...
    do {
        ...
        fprintf(stderr,
                "[RUN #%u %.0f%%, %3u secs] %2u threads: %11lu ops, %7lu (avg: %7lu) ops/sec, %s/sec (avg: %s/sec), "
                "%5.2f (avg: %5.2f) msec latency\r",
                run_id, progress, (unsigned int) (duration / 1000000), active_threads, total_ops, cur_ops_sec, ops_sec,
                cur_bytes_str, bytes_str, cur_latency, avg_latency);
